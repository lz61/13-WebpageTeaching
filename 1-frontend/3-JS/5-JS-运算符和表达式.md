1.JS语言常用运算符:

```
= + - * / % ++ -- == != ,
```

注1: 类似数学上的函数一样,每种运算符在参与运算后都会有一个返回值,例如:

```
1+2; // 返回3
// 1+2等价于3
2*4; // 返回8
// 2*4等价于8
```





2.=运算符

作用: 赋值

```
// 实例:
a=1; // 令a=1,返回1
// 语法:
[变量]=[值];
// =运算符的返回值:
右边的值.
```

注1: 这里=号的作用是赋值,不能用于判断两个值是否相等!



3.+、-、*、/运算符: 均同实数算法

```
b=1;
c=2;
a=b+c; // a=3
a=b-c; // a=-1
a=b*c; // a=2
a=b/c; // a=0.5
```

注1: 事实上,在操作数非数值型数据时,+号有比较麻烦的算法,即运算符重载.这里先略过不讲.



5.%运算符

注1: 作者不会操作数中含有负数的情况,请不要乱用.

注2: 对于操作数均为正数的情况,支持任意正实数A%任意正实数B,并获得A除以B得到的余数.

```
a=3%2; // a=1
a=2.5%2; // a=0.5(可能不完全)
```

注3: 

6.++、--运算符效果

```
var a=3;

var b = a++; // 先返回a,a再++;
// b=3,a=4;

var b = ++a; // 先a++,再返回a;
// b=4,a=4;

var b = a--;
// b=3,a=2;

var b =--a;
// b=2,a=2
```





7.逗号的作用之一: 分割语句,顺次执行,返回最后一个表达式的结果.(不重要)

```
var sum=5;
var pad=5;
pad = ++sum, pad++, ++pad;
// 相当于:
// pad = ++sum;
// pad++;
// ++pad;
console.log("pad=",pad);
console.log("sum=",sum);
```

A.从左向右依次执行

B.整个表达式的结果是最后一个表达式的结果

答案:

```
// pad = ++ sum; pad++; ++pad;

// pad=6,sum=6

// pad=8,sum=6
```



8.逗号的作用之二: 同时申请多个变量

```js
var x,y,z;
// 等价语句:
var x;
var y;
var z;
```



9.题目: 逗号的作用:

```js
var a = 10;
// 区分如下两句话
var b = (a++, a+3, a-3);

var b = a++ , a+3 , a-3 ;
// var b = a++;
// a+3;
// 申请变量并输出: 可以与字符串拼接
console.log("b=",b);
```

答案: "b= 8"





9.表达式:

```
var a;
var b;
则表达式a=2,b=5,b++,a+b的值是
A.5 B.6 C.7 D.8
```

答案: D



10.`+=`,`-=`,`*=`,`/=`,`%=`各自作用

```
a+=3; // 等价:  a=a+3;

a/=2; // 等价: a=a/2;

a%=2; // 默认: a是整型 等价: a=a%2
```



9.==运算符

作逻辑判断,返回值: true or false

在条件判断时比较有用.



10.真题

若变量均已正确定义并赋值,以下合法的赋值语句是:

```
A.x=m%2.5
B.x+n=i
C.x=5=4+1
D.x=y==5
```

答案: D



13.关系运算符(忽略,讲完`6-JS-控制语句-分支`之后再讲)

```
<=,>=,<,>,!=,==
```

A.返回结果: true or false

B.常见错误(Don't be too clever):

```
1<x<2: 到底对于程序来说是什么?
// 是判断1<x<2

// 实例代码
var x = 15;
if(0<x<10){
    console.log("x is between 0 and 10");
}

// 错误,因为计算机会错误理解你的意思.
当true or false和整数相比较时,
强制类型转换: true=1, false =0

// 所以这段代码:
1<x<2: 默认都会执行,因为不论x>1或x<=1, 1<x的返回值都会<2.
```



14.三目运算符(可以自己不写,但是要看得懂别人写出来的三目运算符).

```
x>y?x:y;
[条件判断]?[值1]:[值2];
```



15.真题

![image-20250121164008255](Pics/image-20250121164008255.png)

16.逻辑运算符(推荐先讲控制语句-分支,再讲逻辑运算符&rarr;符合人类的认知理解)

逻辑非!,逻辑与&&,逻辑或||

!: 

&&: 只要有一个表达式为假,则右侧不会再进行计算

||: 只要有一个表达式为真,则右侧不会再进行计算

16.真题

![image-20250121164141690](Pics/image-20250121164141690.png)

```
各自三问的解:
b=3;
b=5;
b=3;
```

17.

![image-20250121164314857](Pics/image-20250121164314857.png)

18.运算符优先级

见表:

![image-20250121171745020](Pics/image-20250121171745020.png)

记忆技巧:

1.单目运算符运算优先级>双目运算符.(单>双)

2.算数运算符优先级>逻辑判断运算符.(算数>逻辑)

3.赋值运算符优先级最低.(=号最后执行)

运算符执行顺序:(不重要)

1.左结合: 从左向右执行

大部分运算符都是从左向右执行的.

2.右结合: 从右向左执行.